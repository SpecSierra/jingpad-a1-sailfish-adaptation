...
class AIDHeaderParser(object):
    ...
    def _handle_aid(self, identifier, value):
        ...
        aid_value = int(aid.normalized_value)

        if aid_value in self._aid_value_to_name and aid.identifier not in AIDHeaderParser._COLLISION_OK:
            raise ValueError(
                'Duplicate aid value "%s" for %s' % (value, identifier))

        self._aid_name_to_value[aid.friendly] = aid
        self._aid_value_to_name[aid_value] = aid.friendly
